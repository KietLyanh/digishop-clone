 ...
 * @author ngotr
 */
@WebServlet(name = "GetAllPackages", urlPatterns = {"/getallpackages"})
public class GetAllPackages extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet GetAllPackages</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet GetAllPackages at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (keyword == null) {
                ...
                List<All_Packages> listSearch = new ArrayList<>();
                for (All_Packages x : list) {
                    if (x.getPackages_name().contains(keyword.toUpperCase())) {
                        listSearch.add(x);
                    }
